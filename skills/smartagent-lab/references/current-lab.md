...
- A third bundle directory `/home/ubuntu/appdsm-node-smartagent2` is also present on the control host. It is not part of the rehearsed live flow; treat it as ad-hoc until reconciled.
...
- **Open drift (April 28, 2026):** on `smartagent-2` (`172.31.1.142`) those two paths are not writable by `ubuntu`. `validate_lab.sh` fails with two `VALIDATION ERROR` lines for that host on every `--require-*` gate. Run `prepare_remote_push.sh --profile <copied-profile> --execute` (it iterates every managed host and is idempotent on the three already-writable hosts) before depending on the latest-bundle remote-push path. The other three managed hosts (`smartagent-1`, `smartagent-3`, `smartagent-4`) currently pass the writability check.
